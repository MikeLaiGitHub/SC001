...
def get_average(pixels):
    ...
    for pixel in pixels:
        red_avg += pixel.red
        green_avg += pixel.green
        blue_avg += pixel.blue
    rgb = [red_avg/len(pixels), blue_avg/len(pixels), green_avg/len(pixels)]
    return rgb
...
def solve(images):
    ...
    for x in range(width):
        for y in range(height):
            process = get_best_pixel(list(pic.get_pixel(x, y) for pic in images))
            result.set_rgb(x, y, process.red, process.green, process.blue)
    # ----- YOUR CODE ENDS HERE ----- #
